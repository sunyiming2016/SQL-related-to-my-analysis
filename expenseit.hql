	---the code to test the company product adoption probability(defined by the percentage of users with mobile activities who needed receipt attachment used the expenseit at least once )
...
where 
        mobile is not null and is_receipt_required='Y' and expenseit_enabled=1---filter user from expenseit enabled company who had mobile activities and receipt was required
group by 
        user_id,
        entity_id
)
...
select 
       ...
       percentile (adoption_probability,0.5) as median_adoption
from b
...
select 
        er.user_id,
        er.entity_id,
        sum(case when er.submit_dttm > first_date and er.submit_dttm<= dateadd( first_date, interval 3 months) then 1 else 0 end )as has_another  --whether the user has  at least one another expenseit supported report wihin 3 months after the first expenseit report submittion
from
